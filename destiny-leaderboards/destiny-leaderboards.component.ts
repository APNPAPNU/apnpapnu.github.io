...
import { LoggerService } from '@ascendant/core';
...
export class DestinyLeaderboardsComponent implements OnInit {
  ...
  constructor(public bungie: BungieService, private logger: LoggerService, private cd: ChangeDetectorRef) { }
    ...
    this.bungie.getRoster().pipe(take(1)).subscribe((response: any) => {
      if (response) {
        this.bungie.getLeaderboards(response).pipe(take(1)).subscribe(leaderboards => {
          ...
          this.logger.log('LEADERBOARDS', this.kdLeaderboard);
