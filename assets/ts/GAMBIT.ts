...
import { BasicInfoService, LoggerService } from '@ascendant/core';

@Injectable({
  providedIn: 'root'
})
export class BungieService {
  constructor(private http: HttpClient, private info: BasicInfoService, private logger: LoggerService) { }
  ...
  private httpOptions = {
    headers: new HttpHeaders().set('X-API-KEY', this.info.bungieApiCredentials.key)
  };

  public getRoster(): Observable<RosterItem[]> {
    const url = `${this.rootUrl}/groupv2/${this.info.bungieApiCredentials.clanId}/members/`;
    if (!this._roster) {
          ...
          if (data) {
            data.forEach(player => {
              if (player.destinyUserInfo && player.bungieNetUserInfo) {
                ...
                this.logger.warn('Unable to retrieve account details for ' + player.destinyUserInfo.displayName);
              }
            });
            roster.sort((a, b) => {
  ...
  public generateDestinyProfileById(destinyId: string, bungieId: string, platform: number): Observable<DestinyProfile> {
    this.logger.log(`generateDestiyProfileById() called for destinyId: ${destinyId} and bungieId ${bungieId}`);
    return forkJoin(
  ...
  public getDestinyProfileById(destinyId: string, platform: number): Observable<{ name, lastPlayed, characterIds }> {
    ...
    return this.http.get(url, this.httpOptions).pipe(
      map((response: any) => {
        this.logger.log('D2 PROFILE', response);
